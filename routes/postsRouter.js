...
router.post("/", async (req, res) => {
  try {
    ...
    if (!text || !user_id) {
      res
        .status(400)
        .json({ error: "You must submit both text and a valid user id." });
    } else {
    ...
    res
      .status(500)
      .json({ error: "There was an error submitting the new post." });
  }
});
  ...
  try {
    ...
    if (!text) {
      ...
      if (post) {
        Posts.update(id, req.body)
          .then(updated => {
            if (updated === 1) {
              ...
              res
                .status(500)
                .json({ error: "There was an error updating the post." });
            }
          })
          .catch(err => {
            res
        ...
        res
          .status(404)
          .json({ error: "A post with the specified ID does not exist." });
      }
    }
  } catch {
